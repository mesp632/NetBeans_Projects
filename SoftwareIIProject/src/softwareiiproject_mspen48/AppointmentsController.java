 ...
 */
package softwareiiproject_mspen48;
...
//a lambda expression in the listener is the shortest and simplest(makes code more readable) way to change the old values to any new ones that appear
    ...
    private List<AppointmentDetails> parseApptList( Long wmview) {
        ...
        PreparedStatement statement = DBConnection.getConn().prepareStatement("SELECT appointment.appointmentid, appointment.customerid, appointment.userid, appointment.title,"
            ...
            while (rs.next()) {
                ...
                tstart=rs.getString("appointment.start");//Timestamp in UTC
                tend=rs.getString("appointment.end");
                
                System.out.println("tstart" + tstart);
               //Instant timestamp = tend.toInstant();//convert timestamp to instant
               
               
                ZonedDateTime testst= LocalDateTime.parse(tstart,DateTimeFormatter.ofPattern("yyyy-MM-dd kk:mm:ss")).atZone(ZoneId.of("UTC"));//convert instant to zoneddatetime
                System.out.println("tstst"+testst);
                //ZonedDateTime UTCst = testst.withZoneSameLocal(ZoneId.of("UTC"));//change timezone to UTC
                ZonedDateTime lst = testst.withZoneSameInstant(currentZone);//convert UTC datetime to localZoneID datetime
                System.out.println("lst" + lst);
                String lst1 = lst.format(DateTimeFormatter.ofPattern("yyyy-MM-dd kk:mm:ss"));
                System.out.println(lst1);
                
                LocalDateTime startldt =  LocalDateTime.parse(lst1,DateTimeFormatter.ofPattern("yyyy-MM-dd kk:mm:ss"));//variable for start localdateTime
               ...
               // Instant etimestamp = tend.toInstant();//convert timestamp to instant
                ZonedDateTime etestst= LocalDateTime.parse(tend,DateTimeFormatter.ofPattern("yyyy-MM-dd kk:mm:ss")).atZone(ZoneId.of("UTC"));//convert instant to zoneddatetime
                ZonedDateTime eUTCst = etestst.withZoneSameInstant(currentZone);//change timezone to UTC
               
                String elst = eUTCst.format(DateTimeFormatter.ofPattern("yyyy-MM-dd kk:mm:ss"));//convert UTC datetime to localZoneID datetime
                LocalDateTime endldt = LocalDateTime.parse(elst,DateTimeFormatter.ofPattern("yyyy-MM-dd kk:mm:ss"));//variable for end localdateTime
    ...
    public void addAppointment(ActionEvent event) throws IOException, SQLException{
        ...
        ZonedDateTime endUTC = zonedStartDateTime.plusMinutes(Integer.parseInt(ApptLengthField.getText()));
        
        Timestamp startUTCTimestamp = Timestamp.valueOf(startUTC);//convert localDateTime to Timestamp variable to insert into sql
        System.out.println("startUTCTimestamp: "+startUTCTimestamp);
    ...
    if (startTime.isAfter(time1) && endTime.isBefore(time2)) {
        ...
        String insertAppt = "INSERT INTO appointment (appointmentid,customerid,userid,title,description,location,contact,type,url,start ,end ,createdate,createdby,lastupdate,lastupdateby)" +
           ...
           try {
            ...
            ps.setString(6,type);
            ps.setTimestamp(7, Timestamp.valueOf(zonedStartDateTime.toLocalDateTime()));
            ps.setTimestamp(8, Timestamp.valueOf(endUTC.toLocalDateTime()));
            ps.execute();
            ErrorNotice.setText("");
    ...
    private void editAppointment(ActionEvent event) throws IOException, SQLException{
        ...
        Timestamp startUTCTimestamp = Timestamp.valueOf(startUTC);//convert localDateTime to Timestamp variable to insert into sql
        Timestamp endUTCTimestamp = Timestamp.valueOf(endUTC);
        ZonedDateTime endTimez = zonedStartDateTime.plusMinutes(Long.parseLong(EditApptLengthField.getText()));
    ...
    if (startTime.isAfter(time1) && endTime.isBefore(time2)) {
        ...
        String insertAppt = "UPDATE appointment SET title = ?,description = ?,type = ?,start=?,end=?,lastupdate=now() WHERE appointmentid=?";
           try {
            ...
            ps.setString(3,type);
            ps.setTimestamp(4, Timestamp.valueOf(zonedStartDateTime.toLocalDateTime()));
            ps.setTimestamp(5, Timestamp.valueOf(endTimez.toLocalDateTime()));
            ps.setInt(6,apptid);
